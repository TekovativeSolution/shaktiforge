...
class QualityCheckWizard(models.TransientModel):
    ...
    def confirm_measure(self):
        ...
        if self.measure_on == 'move_line':
            ...
            self.current_check_id.tested_quantity = self.qty_tested - self.qty_failed
            self.current_check_id.fail_quantity = self.qty_failed
    ...
    def do_pass(self):
        ...
        self.current_check_id.tested_quantity = self.qty_tested
        return self.action_generate_next_window()
    ...
    def confirm_fail(self):
        ...
        if self.measure_on == 'move_line':
            self.current_check_id._move_line_to_failure_location(self.failure_location_id.id, self.qty_failed)
            self.current_check_id.fail_quantity = self.qty_failed
            self.current_check_id.tested_quantity = self.qty_to_test - self.qty_failed
        return self.action_generate_next_window()
