...
class SaleOrder(models.Model):
    ...
    approval_level_id = fields.Many2one(
        'sh.sale.approval.config', string="Approval Level",)
    ...
    def _can_be_confirmed(self):
        return super()._can_be_confirmed() or self.state == 'waiting_for_approval'
                ...
                if users:
                    for user in users:
                        ...
                        self.env['bus.bus']._sendone(user.partner_id, 'simple_notification', {
                            ...
                            'message': _('You have approval notification for sale order %s' % (self.name))
                        })
    ...
    def onchange_approval_level_id(self):
        if self.team_id and self.team_id.sale_approval_config_id:
            self.approval_level_id = self.team_id.sale_approval_config_id.id
            ...
            if self.user_id:
                ...
                self.env['bus.bus']._sendone(self.user_id.partner_id, 'simple_notification',
                                             {'title': _('Notitification'),
                                              'type': 'info',
                                              'sticky': True,
                                              'message': _('Dear SalesPerson your order %s is confirmed' % (self.name))
                                              })
