...
class Company(models.Model):
    ...
    def _default_spiffy_favicon(self):
        # image_path = file_path('spiffy_theme_backend/static/description/app-drawer-bg-image.png')
        image_path = file_path('spiffy_theme_backend/static/description/Shield.png')
        with tools.file_open(image_path, 'rb') as f:
            return base64.b64encode(f.read())

    def _default_login_bg_img(self):
        image_path = file_path('spiffy_theme_backend/static/description/login screen background.jpg')
        with tools.file_open(image_path, 'rb') as f:
            return base64.b64encode(f.read())

    def _default_menubar_logo(self):
        image_path = file_path('spiffy_theme_backend/static/description/Menubar Logo.png')
        with tools.file_open(image_path, 'rb') as f:
            return base64.b64encode(f.read())

    tab_name = fields.Char(string="Backend Tab Name", default="Teknovative Solution", readonly=False)
    backend_theme_level = fields.Selection([
        ('user_level', 'User Level'),
        ('global_level', 'Global Level')],
        default="global_level", required=True, string="Backend Theme Level", readonly=False)
    ...
    login_page_style = fields.Selection([
		...
		default="login_style_2", required=True, string="Login Styles", readonly=False)
    
    # login_page_background_img = fields.Binary('Login Background Image', readonly=False, store=True)
    login_page_background_img = fields.Binary('Login Background Image', readonly=False, store=True, default=_default_login_bg_img)
    ...
    login_page_text_color = fields.Char('Login Text Color', default="#242424", readonly=False)
    ...
    # spiffy_favicon = fields.Binary(string="Backend Tab Favicon", readonly=False, default=_default_spiffy_favicon)

    @api.model
    def _default_logo(self):
        with tools.file_open('spiffy_theme_backend/static/description/Teksol logo.png', 'rb') as f:
                return base64.b64encode(f.read())
    ...
    backend_menubar_logo = fields.Binary(
        string="Menubar Logo",  readonly=False, default=_default_menubar_logo)
    ...
    firebase_server_key = fields.Char(string='Firebase key', default="AAAAl184GwM:APA91bEm2nxRGsz0GMeV7TDQ1OjwzKKkjmPOxTJLEFy3LaE7iowj3JN8WcNAuGcdDVDcwJpnkp-T5Uy0B_4dN6ytRN77F1CV_u10JdV9phduFe_qrORu_3FRmo2g33__mVBZH9yEFwhC",
                                    readonly=False)
    firebase_key_file = fields.Binary('Firebase Key File', default=lambda self:self._get_firebase_records(), readonly=False)
    
    def _get_firebase_records(self):
        json_path = file_path('spiffy_theme_backend/static/description/firebase-key/spiffy-99038-firebase-adminsdk-tmssr-26241eaaa2.json')
        with tools.file_open(json_path, 'rb') as f:
            return base64.b64encode(f.read())
