...
class ResConfig(models.TransientModel):
    ...
    def _get_default_image_background(self):
        image_path = "/run/media/shakti/odoo-erp/workspace/shaktiforge/spiffy_theme_backend/static/src/company_images/login_background.jpg"
        with open(image_path, "rb") as f:
            return base64.b64encode(f.read())

    login_page_background_img = fields.Binary(
        related='company_id.login_page_background_img', string="Login Background Image",attachment=True,default=_get_default_image_background,readonly=False)
    ...
    def _get_default_image(self):
        image_path = "/run/media/shakti/odoo-erp/workspace/shaktiforge/spiffy_theme_backend/static/src/company_images/Shield.png"
        with open(image_path, "rb") as f:
            return base64.b64encode(f.read())
    ...
    def _get_default_menu_logo(self):
        image_path = "/run/media/shakti/odoo-erp/workspace/shaktiforge/spiffy_theme_backend/static/src/company_images/menubar_logo.png"
        with open(image_path, "rb") as f:
            return base64.b64encode(f.read())
    backend_menubar_logo = fields.Binary(
        related='company_id.backend_menubar_logo', string="Menubar Logo", readonly=False,attachment=True,default=_get_default_menu_logo)
    
    backend_menubar_logo_icon = fields.Binary(
        related='company_id.backend_menubar_logo_icon', string="Menubar Logo Icon", readonly=False,attachment=True,default=_get_default_menu_logo)
    ...
    firebase_key_file = fields.Binary('Firebase Key File', readonly=False, related='company_id.firebase_key_file')
    ...
    def get_values(self):
        ...
        firebase_key_file=self.env['ir.config_parameter'].get_param('firebase_key_file')
        if not firebase_key_file:
            ...
            with tools.file_open(json_path, 'rb') as f:
                firebase_key_file = base64.b64encode(f.read())
        res.update(firebase_server_key=firebase_server_key,firebase_key_file=firebase_key_file)
        return res